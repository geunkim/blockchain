...
개별 주장들은 주체에 대한 정보의 그래프 표현을 위해 합칠 수 있다. 
이 그래프는 주체와 다른 주체 또는 데이터와의 관계로 구성된 정보의 네트워크이다.
다음은 앞의 예(}Pat가 Example University의 졸업생이다")에 
"Pat가 Sam을 안다"와 "Sam의 직업이 교수다"라는 주장을 함께 나타낸 그래프이다. 

![image](./claims_network.png)

이제까지 주장과 정보의 그래프에 대한 개념이 소개되었다. 주장을 신뢰할 수 있으려면 더 많은 정보가 그래프에
추가될 것으로 예측된다. 

### Credentials (자격증명)
 
자격증명은 같은 엔티티에 의해 생성된 주장들의 집합이다. 
자격증명은 식별자(identifer)와 발급자(issuer), 만료 날짜와 시간, 대표 이미지, 검증 목적으로 사용되는 공개키, 해지 메커니즘 등 자격증명의 속성을 기술하는 메타데이터를 포함할 수 있다. 
메타데이터는 발급자가 서명할 수 있다. 
검증가능한 자격증명은 발급자를 암호적으로 증명되는 메터데이터와 변조을 감지할 수 있는 주장들의 집합이다. 

![image](./verifiable_credential.png) 
(검증가능한 자격증명의 기본 컴포넌트 - source: w3c)

검증가능한 자격증명의 예는 디지털 직원 신분증, 디지털 출생 증명서, 디지털 교육 증명서가 있다. 

자격증명 식별자는 종종 자격증명의 특정한 인스턴스를 식별하는데 사용된다. 식별자들은 연관 관계를 설졍하는데 사용할 수도 있다. 연관 관계를 최소화려는 
보유자는 자격증명 식별자를 밝히지 않고 선택적 공개 체계를 사용하는 것이 바람직하다. 

앞에 나타낸 검증가능한 자격증명의 기본 컴포넌트로 주장이 정보의 그래프로 구성되고 검증가능한 자격증명을 구성하는 방법에 대한 상세한 내용을 추상화한다.

다음은 일반적으로 최소 두 개의 정보 그래프로 구성된 검증가능한 자격증명으로 앞의 그림보다 더욱 완벽하게 보여준다.
첫 번째 그래프는 자격증명 메타데이터와 주장을 포함하는 검증가능한 자격증명 자체를 표현한다.
두 번째 그래프는 보통 디지털 서명인 디지털 증명을 표현한다. 

![image](./info_graph_verifiable_credential.png)

서로 관련될 필요가 없는 다른 주체들에 관한 여러 주장을 포함하는 결혼 증명서와 같은 자격증명을 가질 수 있다. 

자격증명이 발급된 엔티티에 대한 어떠한 주장을 포함하지 않은 자격증명을 포함할 수 있다. 
예로 특정 강아지에 대한 주장을 포함한 자격증명은 강아지에게 발급되는 자격즘영이 아니라 강아지의 주인에게 발급되는 것이다.

앞의 그림은 검증가능한 자격증영의 기본 컴포넌트로 주장들이 어떻게 정보 그래프로 구성되고 정보 그래프가 검증가능한 자격증명으로 어떻게 구성되는지에
대한 상세한 내용을 추상화하였다.  

### Presentations (표현)

자격증명의 데이터 모델을 설게할 때 주요 설계 특징은 개인정보 보호를 향상시키는 것이다. 
이 기술을 사용하는 엔티티가 주어진 상황에 적절한 개인의 일부만 표현할 수 있어야 한다.
우리 모습의 일부분을 표현하는 것을 검증가능한 표현이라 한다. 
다른 인물의 예에는 개인의 전문 페르소나(persona:모습), 온라인 게임 페르소나, 가족 페로소나 또는 익명 페로소나가 있다. 

검증가능한 표현은 하나이상의 검증가능한 자격검증의 데이터를 표현하고 데이터의 소유권을 검증하는 벙법으로 패키지된다. 
검증가능한 자격증명은 직접 표현된다면, 검증가능한 자격즘명은 검증가는 표현이 된다. 
검증가능한 자격증명으로 부터 도출된 데이퍼 포맷은 암호적으로 검증이 가능하나 검증가능한 자격증명을 포함하지 않아도 
검증가능한 표현일 수 있다.

표현의 데이터는 종종 같은 주체에 관한 것으로 여러 발급자에 의해서 발급되었을 수 있다.  
이들 정보의 집합체는 사람, 조직, 엔티티의 측면을 표현한다. 

다음 그림은 검증가능한 표현의 컴포턴트를 보여주나 검증가능 자격증명이 어떻게 정보 그래프로 구성되고 정보 그래프가 검증가능한 표현으로 어떻게 구성되는지에
대한 상세한 내용을 추상화하였다.  


![image](./verifiable_presentation.png) 


다음은 일반적으로 최소 네 개의 정보 그래프로 구성된 검증가능한 표현으로 앞의 그림보다 더욱 완벽하게 보여준다.
첫 번째 그래프는 표현 메타데이터를 포함하는 검증가능한 표현 자체를 표현한다.
그래프의 **verifiablePresentation** 속성은 하나 이상의 검증가능 자격증명(자격증명 메타데이터와 주장)을 참조한다. 
세번쨰 그래프는 디지털 서명인 자격증명 그래프 증명을 표현한다.
네번쨰 그래프는 디지털 서명인 표현 그래프 증명을 표현한다. 

![image](./info_graph_verifiable_presentation.png)


항상 관련이 있을 필요는 없는 다른 주체에 관한 여러 자격증명을 이용하는 비지니스 페르소나와 같은 표현을 가질수 있다. 
...
## Standards related to Verifiable Credential 
